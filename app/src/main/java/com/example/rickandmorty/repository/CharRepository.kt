...
class CharRepository(private val retrofitCharNetService: RetrofitCharNetService) {

     fun getPagedList(): Flow<PagingData<Character>> {

        return Pager(config = PagingConfig(pageSize = 20, enablePlaceholders = false, maxSize = 20*3),
            pagingSourceFactory = {CharacterPagingSource(retrofitCharNetService)}).flow
